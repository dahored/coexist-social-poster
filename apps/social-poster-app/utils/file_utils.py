...
# Global paths
UPLOADS_DIR = get_path_from_base("public", "uploads")
ASSETS_DIR = get_path_from_base("assets")

IMAGES_PATH = os.path.join(UPLOADS_DIR, "images")
VIDEOS_PATH = os.path.join(UPLOADS_DIR, "videos")
TEMPS_PATH = os.path.join(UPLOADS_DIR, "temps")
...
class FileHandler:
    def __init__(self):
        ...
        self.temps_path = TEMPS_PATH
        self.logos_base_path = LOGOS_DIR
        self.backgrounds_base_path = BACKGROUNDS_DIR
    ...
    def add_watermark(self, canvas, watermark_path, width, height, show_watermark_name=False, theme='light'):
        try:
            ...
            font_path = os.path.join(FONTS_DIR, os.getenv('FONT_FAMILY', 'CascadiaMonoPL.ttf'))
    ...
    async def generate_image(self, data, id, theme='light', width=1024, height=1024, font_size=None, font_ratio=24, text_scale=1.0):
        ...
        if theme == 'dark':
            watermark_path = os.path.join(LOGOS_DIR, os.getenv("WATERMARK_IMAGE_LIGHT", "logo_coexist_white_watermark.png"))
        else:
            watermark_path = os.path.join(LOGOS_DIR, os.getenv("WATERMARK_IMAGE_DARK", "logo_coexist_black_watermark.png"))
